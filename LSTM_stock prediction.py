...
class LSTM_stock_predict:
    ...
    def Model_train(self, data_path, ):
        self.df = pd.read_csv(data_path + 'Shenzhen Internet Index.csv')
        
        self.X = self.df[['日期','成交量','成交金额','今开盘指数','最低价指数','最高价指数','今收盘指数','昨收盘指数']].values #日期的0对应2015年6月8日
        self.Y = self.df[['涨跌幅']].values
        
        self.X_train, self.X_test, self.Y_train, self.Y_test = train_test_split(self.X, self.Y, test_size=0.15, random_state=77)
        ...
        self.model.save('LSTM_stock prediciton.h5')
        ...
        axs[0, 1].plot([min_val, max_val], [min_val+0.03*(max_val-min_val), max_val+0.03*(max_val-min_val)], 'k--', label='+2%')
        axs[0, 1].plot([min_val+0.03*(max_val-min_val), max_val], [min_val, max_val-0.03*(max_val-min_val)], 'k--', label='-2%')
        ...
        axs[1, 0].plot(self.df['日期'], self.df['涨跌幅'],label='Raw data')
        axs[1, 0].plot(self.df['日期'][-(len(self.Y_predict)+1):-1], self.Y_predict[:,0],label='Predicted data')
        axs[1, 0].set_xlabel('Duration (day)')
        axs[1, 0].set_ylabel('Variation (%)')
        axs[1, 0].set_title('Raw data vs. Prediction')
        axs[1, 0].legend()
        
        axs[1, 1].plot(self.df['日期'][-(len(self.Y_predict)+1):-1], self.df['涨跌幅'][-(len(self.Y_predict)+1):-1],label='Raw data')
        axs[1, 1].plot(self.df['日期'][-(len(self.Y_predict)+1):-1], self.Y_predict[:,0],label='Predicted data')
    ...
    def Model_load(self, data_path, data):
        ...
        self.model = load_model(data_path+'LSTM_stock prediciton.h5')
...
if __name__ == '__main__':
    ...
    predictor.Model_train(data_path)
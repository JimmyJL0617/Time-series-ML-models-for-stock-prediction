...
class LSTM_stock_predict:
    ...
    def Model_train(self, data_path, ):
        self.df = pd.read_csv(data_path + 'yahoo NASDAQ historical data 5y.csv')
        
        train_size = int(len(self.df)*0.85)
        self.df_train = self.df.iloc[:train_size]
        self.df_test = self.df.iloc[train_size:]
        
        self.X_train = self.df_train[['Date','Open','High','Low','Volume']].values #The Day 0 of the Date refers to the first day of the data
        self.Y_train = self.df_train[['Close']].values
        
        self.X_test = self.df_test[['Date','Open','High','Low','Volume']].values #The Day 0 of the Date refers to the first day of the data
        ...
        self.model.save('LSTM_stock prediciton_NASDAQ.h5')
        ...
        axs[1, 0].plot(self.df['Date'], self.df['Close'],label='Raw data')
        ...
        axs[1, 1].plot(self.df['Date'][-(len(self.Y_predict)+1):-1], self.df['Close'][-(len(self.Y_predict)+1):-1],label='Raw data')
    ...
    def Model_load(self, data_path, data):
        ...
        self.model = load_model(data_path+'LSTM_stock prediciton_NASDAQ.h5')
